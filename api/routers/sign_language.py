...
async def refresh_pipeline(force: bool = True):
    ...
    try:
        ...
        # [추가] 모션 토크나이저 사전도 Supabase 최신 상태로 갱신
        motion_tokenizer.refresh()
        tok_stats = motion_tokenizer.stats()
        ...
        return {
            ...
            "count": count,
            "tokenizer": tok_stats,
            "message": f"{count}개 수어-감정 데이터 처리 성공 ({mode}) / 토크나이저 사전 {tok_stats['total']}개 단어 갱신 완료"
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
